...
def test_get_user_by_name(db):
    crud.create_user(db, schemas.UserCreate(name="finder", password="666"))
...
def test_get_users_list(db):
    crud.create_user(db, schemas.UserCreate(name="A", password="666"))
    crud.create_user(db, schemas.UserCreate(name="B", password="666"))
    users = crud.get_users(db)
    assert len(users) >= 2


def test_update_user_password(db):
    user = crud.create_user(db, schemas.UserCreate(name="passuser", password="oldpass"))
...
def test_delete_user(db):
    user = crud.create_user(db, schemas.UserCreate(name="deleteme", password="666"))
...
def test_order_lifecycle(db):
    user = crud.create_user(db, schemas.UserCreate(name="orderuser", password="666"))
