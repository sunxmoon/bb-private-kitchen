...
def test_post_without_csrf_is_rejected(client, db):
    ...
    response = client.post("/login", data={"name": "testuser", "password": "testpass666"}, follow_redirects=False)
...
def test_post_with_wrong_csrf_is_rejected(client, db):
    ...
    response = client.post(
        "/login", data={"name": "testuser", "password": "testpass666", "csrf_token": "wrong-token"}, follow_redirects=False
    )
    assert response.status_code == 403
...
def test_post_with_correct_csrf_succeeds(client, db):
    ...
    response = client.post(
        "/login", data={"name": "testuser", "password": "testpass666", "csrf_token": "valid-token"}, follow_redirects=False
    )
